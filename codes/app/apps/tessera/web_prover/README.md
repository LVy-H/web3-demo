...
- **web** — loaded via `web/index.html` `<script>`, called through
  `dart:js_interop` (`packages/core_crypto/lib/proof/proof_service_web.dart`).
- **mobile** — the headless-WebView prover serves the bundled asset COPY at
  `packages/core_crypto/assets/zk/zkprover.js` (a parity test keeps it
  byte-identical to `../web/zkprover.js` — re-copy after every rebuild).
- **desktop** — `desktop_prover.mjs` eval's the same bundle under Node;
  `packages/core_crypto/lib/proof/proof_service_desktop.dart` drives it
  over stdio.
...
```sh
cd codes/app/apps/tessera/web_prover
