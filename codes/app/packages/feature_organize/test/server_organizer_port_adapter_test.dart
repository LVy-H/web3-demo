...
void main() {
  group('deployPoll', () {
    test('POSTs the open-ballot create body, then opens, and records the id',
        () async {
      ...
      final adapter = ServerOrganizerPortAdapter(
        client: _client(
          MockClient((r) async {
            ...
            return http.Response(
              jsonEncode({'id': 'd', 'state': r.url.path.endsWith('open') ? 'open' : 'draft'}),
  ...
  group('reads', () {
    ...
    test('fetchTurnout falls back to the public view turnout on a 403',
        () async {
      final adapter = ServerOrganizerPortAdapter(
        client: _client(
          MockClient((r) async {
    ...
    test('fetchResults maps optionScores to labelled OrganizerResults',
        () async {
      final adapter = ServerOrganizerPortAdapter(
        client: _client(
          MockClient((r) async {
    ...
    test('loadCreatedPolls maps each id to a RunPollSnapshot, skipping 404s',
        () async {
