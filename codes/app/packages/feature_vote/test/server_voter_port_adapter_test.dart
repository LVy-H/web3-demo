...
void main() {
  group('fetchSnapshot', () {
    test('maps state strings to the phase int (draft→0, open→1, closed→2)',
        () async {
  ...
  group('open-mode gates', () {
    test('checkEligibility is always true (open mode)', () async {
      final adapter = _adapter(MockClient((r) async => http.Response('{}', 200)));
      expect(await adapter.checkEligibility(_view()), isTrue);
    });

    test('generateProof returns the placeholder without any network call',
        () async {
      var called = false;
      final adapter = _adapter(
        MockClient((r) async {
  ...
  group('relayBallot payload mapping', () {
    ...
    test('survey answer words → {kind:survey, choices:[selected indices]}',
        () async {
      final body = await castBodyFor(
  ...
  group('relayBallot receipt + errors', () {
    test('maps the server receipt → VoterReceipt (nullifier=hash, tx=root)',
        () async {
      final adapter = _adapter(
        MockClient(
          (r) async => http.Response(
