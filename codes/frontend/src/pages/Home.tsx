    ...
    return (
        <div className="space-y-12">
            <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                ...
                {isPending && <p className="text-amber-600 text-sm font-medium mb-4 animate-pulse">Submitting transaction...</p>}
                {isConfirming && <p className="text-amber-600 text-sm font-medium mb-4 animate-pulse">Creating poll transaction pending...</p>}
                {isSuccess && <p className="text-emerald-600 text-sm font-medium mb-4">Poll created successfully!</p>}
                ...
                <form onSubmit={handleCreatePoll} className="space-y-5">
                    ...
                    <button type="submit" disabled={!address || isPending || isConfirming} className="w-full sm:w-auto px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white transition-colors rounded-xl text-sm font-medium shadow-sm whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed">
                        {!address ? 'Connect Wallet First' : isPending ? 'Submitting...' : isConfirming ? 'Confirming...' : 'Create Poll'}
                    </button>
                </form>
            </section>
                            ...
                            >
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-xs font-bold px-2 py-0.5 bg-blue-100 text-blue-700 rounded-md uppercase">
                                            {poll.moduleType}
                                        </span>
                                    </div>
